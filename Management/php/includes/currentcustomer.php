    ...
    ?>
        ...
        <div class="bottom">
            <ul class="orders">
                <?php      
            $total = 0;            
            $orders = $db->query("SELECT item_name, price, qty, total FROM (SELECT id, item_name, price, SUM(qty) AS qty, SUM(total) AS total FROM orders WHERE customer_id=? GROUP BY id, item_name) orders;", $current['customer_id'])->fetchAll(); 
