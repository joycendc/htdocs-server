    ...
    ?>
        ...
        <div class="bottom">
            <ul class="orders">
                <?php      
            $total = 0;            
            $orders = $db->query("SET sql_mode = ''; SELECT item_name, price, qty, total FROM (SET sql_mode = ''; SELECT id, item_name, price, SUM(qty) AS qty, SUM(total) AS total FROM orders WHERE customer_id=? GROUP BY item_name) orders;", $current['customer_id'])->fetchAll(); 
